...
public class ErrorHandlingIntegrationTests : IntegrationTestBase
{
        ...
        ILoraDbClientFixture fixture)
    {
        ...
        {
            var exception = await Assert.That(() => client.ExecuteAsync("THIS IS NOT CYPHER"))
                .ThrowsException();
        ...
        ILoraDbClientFixture fixture)
    {
        await WithCleanDatabaseAsync(fixture, async client =>
        {
            var exception = await Assert.That(() => client.ExecuteAsync("MATCH (n) RETURN missingVar"))
                .ThrowsException();
        ...
        ILoraDbClientFixture fixture)
    {
        await WithCleanDatabaseAsync(fixture, async client =>
        {
            await Assert.That(() => client.ExecuteAsync("THIS IS NOT CYPHER")).ThrowsException();
