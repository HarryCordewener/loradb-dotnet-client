...
This package redistributes third-party software. The notices below apply to the
redistributed components.

## LoraDB (`lora_ffi` native libraries)

- Component: `runtimes/*/native/lora_ffi.dll`, `liblora_ffi.so`, `liblora_ffi.dylib`
- Upstream project: LoraDB — https://github.com/lora-db/lora
- Built from: the `crates/bindings/lora-ffi` crate at the upstream ref pinned in
  `LoraDb.Client.Native/lora-ffi.version` (see `scripts/build-lora-ffi.sh`)
- Copyright: LoraDB, Inc.
- License: Business Source License 1.1 (SPDX: `BUSL-1.1`)

The full text of the license, with LoraDB, Inc.'s parameters, is reproduced
verbatim below.
